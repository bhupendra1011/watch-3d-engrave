...
function loadObject() {
  ...
  objectLoader.load(
    ...
    function(obj) {
      ...
      minuteHand = object3d.children.forEach(element => {
        element.name === "minute-hand" ? (minuteHand = element) : null;
        element.name === "second-hand" ? (secondHand = element) : null;
        element.name === "hour-hand" ? (hourHand = element) : null;
        element.name === "back-dial" ? (backDial = element) : null;
      });
      backDialMaterial = backDial ? backDial.material : "";
...
function engraveTextOnWatch(val = document.querySelector("#engtxt").value) {
  ...
  // backDialMaterial.map = engravedText;
  // back dial marerial is at 49 positions in scene objecs
